...
if [ -d "$REPO_DIR/.git" ]; then
    info "Repository exists at $REPO_DIR. Syncing to origin/$REPO_BRANCH..."
    # Fetch upstream so origin/$REPO_BRANCH is current.
    git -C "$REPO_DIR" fetch origin "$REPO_BRANCH"

    # Preserve and log any local modifications before resetting.
    # Unlike `pull --ff-only` (which aborts on any local modification, e.g.
    # a stray chmod on bootstrap.sh from a prior run), we stash divergent
    # changes so they're recoverable and visible in the deploy log, then
    # hard-reset tracked files so the deploy always matches upstream.
    # Untracked/gitignored runtime files are untouched by stash/reset:
    # terraform state (*.tfstate*), the .terraform dir, ssh_private_key,
    # manifest.yaml and testbed.yaml — generated by terraform apply and
    # required to survive a re-run of this phase.
    if ! git -C "$REPO_DIR" diff-index --quiet HEAD --; then
        warn "Local changes to tracked files in $REPO_DIR — stashing (diff below):"
        git -C "$REPO_DIR" --no-pager diff HEAD
        if git -C "$REPO_DIR" stash push \
            -m "sunbeam-deployer pre-reset $(date -u +%FT%TZ)"; then
            log "Local changes stashed (recover: git -C $REPO_DIR stash pop)"
        else
            err "Failed to stash local changes — aborting to avoid discarding them."
            exit 1
        fi
    fi

    git -C "$REPO_DIR" reset --hard "origin/$REPO_BRANCH"
    log "Repository synced to origin/$REPO_BRANCH."
else
